...
**构造**：对 $i \in \{0, 1\}$，定义单特征模型 $m_i(x) = g_i(x_j)$，其中 $g_i: \mathbb{R} \to \mathbb{R}$ 满足：

- **局部**：$g_i(t) = h_i(t)$ for all $t \in [x_j - \delta, x_j + \delta]$。
- **远场**：$g_i(t) = \lambda_i$ for all $t \in (a_j, b_j) \setminus [x_j - \delta, x_j + \delta]$，$g_i(t) = 0$ for $t \notin (a_j, b_j)$。

由 $(\star\star)$：

$$\Phi(m_i, \mathbf{d})_j = g_i(x_j) - \mathbb{E}_{\mu_j}[g_i] = h_i(x_j) - \int_{x_j - \delta}^{x_j + \delta} h_i(t) \, d\mu_j(t) - \lambda_i \cdot p$$

其中 $p := \mu_j\big((a_j, b_j) \setminus [x_j - \delta, x_j + \delta]\big) > 0$（由 (A3b)）。

选择

$$\lambda_i = \frac{h_i(x_j) - \int_{x_j - \delta}^{x_j + \delta} h_i(t) \, d\mu_j(t) - v}{p}$$
...
由 (A3a)，上述 $g_i$（连续单变量函数）对应的单特征模型属于 $M_{\text{add}} \subseteq M$。由构造，$m_i \in M_{h_i}^{(j)}$。
...
Step 2 证明了右端已经足以使 spec + sens $\leq 1$。即使 $\Phi$ 在非可加模型上"聪明地"提取了交互信息从而表现更好，$\inf$ 仍然被可加子类中的 adversarial 实例拖至最坏情况。
...
**Counterexample.** 取 $d=2$。对非可加模型定义

$$\Phi(m, \mathbf{d})_1 := m(x) - \mathbb{E}_\mu[m], \qquad \Phi(m, \mathbf{d})_2 := 0$$

对可加模型 $m(x) = m_1(x_1) + m_2(x_2)$，退回

$$\Phi_1 = m_1(x_1) - \mathbb{E}[m_1], \quad \Phi_2 = m_2(x_2) - \mathbb{E}[m_2]$$

此 $\Phi$ 满足 (A2a) completeness（对所有模型，$\Phi_1 + \Phi_2 = m(x) - \mathbb{E}[m]$）和 (A2b) additivity（对可加模型），但在非可加模型上不遵循边际公式——所有归因被倾倒到第一个特征中。
...
**Proposition.** 仅 completeness 不导致不可能性。存在满足 (A2a) + (A3) 且达到 spec + sens $> 1$ 的 $\Phi$。

**Proof (by construction).** 取 $d=2$，定义
...
这满足 (A2a) completeness：$\Phi_1 + \Phi_2 = m(x) - \mathbb{E}[m]$。但不满足 (A2b) additivity：对可加 $m(x) = m_1(x_1) + m_2(x_2)$，$\Phi_1 = m_1'(x_1)$ 不等于 $m_1(x_1) - \mathbb{E}[m_1]$。

此时 $\Phi_1$ 编码了 $m$ 在 $x$ 处沿 $x_1$ 方向的梯度——这是**真正的局部信息**。对两个局部行为 $h_0, h_1$ 满足 $h_0'(x_1) \neq h_1'(x_1)$，对所有 $m \in M_{h_i}^{(1)}$，$\Phi_1 = h_i'(x_1)$（无论远场行为如何）。因此 $I(\Phi) = \mathbb{1}[\Phi_1 = h_1'(x_1)]$ 给出 spec + sens $= 2 > 1$。$\square$
...
**已证明的**:
1. (A2) + (A3) → ¬(A1)：证明完整，无逻辑跳跃。
2. 定理覆盖所有包含可加子类的模型空间（Section 5.3）。
3. 边际公式不能推广到非可加模型（Section 5.4 反例）。
4. Completeness alone 不导致不可能性（Section 5.5 构造）。
...
- Section 5.5 构造了仅满足 (A2a) 且 $\text{IGA} = 2$ 的 $\Phi$。
...
$$\Phi^\alpha(m, \mathbf{d})_j = (1-\alpha) \cdot \nabla_j m(x) + \alpha \cdot \text{SHAP}_j(m, \mu, x)$$

其中 $\alpha \in [0,1]$。$\alpha=0$ 是纯梯度（满足 completeness 但不满足 additivity），$\alpha=1$ 是 SHAP（满足两者）。

3. **Metric**：对每个 $\alpha$，估计 $\text{IGA}(\Phi^\alpha)$——对随机生成的 $(h_0, h_1)$ 对，找使 $\Phi$ 值相同的模型对，计算最优 spec + sens。

4. **预测**：
   - $\alpha = 0$: $\text{IGA} > 1$（梯度编码局部信息）
...
> **注意**：$\Phi^\alpha$ 对中间 $\alpha$ 不满足 completeness（梯度和 SHAP 的 completeness 总量不同），需要归一化。一个更干净的做法是：$\Phi^\alpha_j = (1-\alpha) \cdot \text{IG}_j + \alpha \cdot (\text{SHAP}_j - \text{IG}_j)$，其中 IG = Integrated Gradients（同时满足 completeness）。
